...
def parse_data_bs(filename):
    ...
    for item in items:
        ...
        answer.clear()
        #time.sleep(1)
#parse_data_bs('test15.html')
#f.close()
f=open('q15.csv','r')
lines = f.readlines()
db = sqlite3.connect('inf9gia.db')
cursor = db.cursor()
sql="""DROP TABLE IF EXISTS `question15`"""
cursor.execute(sql)
db.commit()
sql="""CREATE TABLE IF NOT EXISTS `question15` (
...
for line in lines:
    ...
    sql = """INSERT INTO question15(text2, right_answer, memorial)
