...
def parse_data_bs(filename):
    ...
    for item in items:
        ...
        img_c = Image.new("RGB", (338, 130))
        base_img=Image.open('img/base_18.png')
        ...
        # добавляем текст
        for i in range(1,5):
            tds_for=trs[i].find_all('td')
           
            try:
                draw.text((x,y), tds_for[1].text, (0,0,0), font)
            except BaseException:
                pass
            y+=24

        img_c.save('img/z18/'+str(k)+'.png', 'PNG')
        img_c.close()
        base_img.close()
        ...
        memorial=memorial.replace('\n',' ')
        memorial=memorial.replace('  ',' ')
        url_img='img/z18/'+str(k)+'.png'
        f.write(str(k)+';'+url_img+';'+text2+';'+right_answer+';'+memorial+'\n')
        k+=1
        answer.clear()
        #time.sleep(1)
#parse_data_bs('test18.html')
#f.close()
f=open('q18.csv','r')
lines = f.readlines()
db = sqlite3.connect('inf9gia.db')
cursor = db.cursor()
sql="""DROP TABLE IF EXISTS `question18`"""
cursor.execute(sql)
db.commit()
sql="""CREATE TABLE IF NOT EXISTS `question18` (
...
for line in lines:
    ...
    sql = """INSERT INTO question18(file, text2, right_answer, memorial)
