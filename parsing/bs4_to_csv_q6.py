...
def parse_data_bs(filename):
    ...
    for item in items:
        ...
        try:
            ...
            pass
        try:
            file_p='Чертёжнику был дан для исполнения следующий алгоритм:{n}'+'<pre>'+str(texts[7])+'{n}'+str(texts[8])+'{n}'+str(texts[9])+'</pre>'
            if 41<=i<=50:
                file_p='Чертёжнику был дан для исполнения следующий алгоритм:{n}'+'<pre>'+str(texts[7])+'{n}'+str(texts[8])+'{n}'+str(texts[9])+'{n}Конец</pre>'
            if 17<=i<=29:
                file_p='Чертёжнику был дан для исполнения следующий алгоритм:{n}'+'<pre>'+str(texts[7])+'{n}'+str(texts[8])+'</pre>'
        except BaseException:
            pass
        texts.reverse()
        #варианты ответа
        ...
        try:
            ...
            pass
        try:
            file_p=file_p.replace('<p class=\"left_margin\">','')
        except BaseException:
            pass
        try:
        ...
        try:
            ...
            pass
        try:
            file_p=file_p.replace('<!--auto generated from answers-->','')
        except BaseException:
            pass
        ...
        i+=1
        answer.clear()
#parse_data_bs('test6.html')
#f.close()
f=open('q6.csv','r')
lines = f.readlines()
db = sqlite3.connect('inf9gia.db')
cursor = db.cursor()
sql="""DROP TABLE IF EXISTS `question6`"""
cursor.execute(sql)
db.commit()
sql="""CREATE TABLE IF NOT EXISTS `question6` (
...
for line in lines:
    ...
    sql = """INSERT INTO question6(file, text1, text2, right_answer, wrong_answer, memorial)
