...
def parse_data_bs(filename):
    ...
    for item in items:
        ...
        answer.clear()
        #time.sleep(1)
parse_data_bs('test17.html')
f.close()
f=open('q17.csv','r')
lines = f.readlines()
db = sqlite3.connect('inf9gia.db')
cursor = db.cursor()
sql="""DROP TABLE IF EXISTS `question17`"""
cursor.execute(sql)
db.commit()
sql="""CREATE TABLE IF NOT EXISTS `question17` (
...
for line in lines:
    ...
    sql = """INSERT INTO question17(text2, right_answer, memorial)
