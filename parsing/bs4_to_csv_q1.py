...
def parse_data_bs(filename):
    ...
    for item in items:
        ...
        i+=1
        answer.clear()
parse_data_bs('test.html')
f.close()
f=open('q1.csv','r')
lines = f.readlines()
db = sqlite3.connect('inf9gia.db')
cursor = db.cursor()
sql="""DROP TABLE IF EXISTS `question1`"""
cursor.execute(sql)
db.commit()
sql="""CREATE TABLE IF NOT EXISTS `question1` (
...
for line in lines:
    ...
    sql = """INSERT INTO question1(text2, right_answer, wrong_answer, memorial)
