...
class MySQL:
    ...
    def get_user(self,chat_id):
        ...
        with self.connection:
            self.cursor.execute("SELECT * FROM question WHERE vk_id="+str(chat_id)+";")
    ...
    def update_question_result(self, chat_id, question_number,result,type_q):
        ...
        with self.connection:
            self.cursor.execute("UPDATE question SET z"+str(question_number)+"_"+type_q+"="+str(result)+" WHERE vk_id = "+str(chat_id)+";")
            self.connection.commit()

    def update_time(self, chat_id, time_q):
        """записываем результат в базу данных"""
        with self.connection:
            self.cursor.execute("UPDATE question SET last_time=\'"+str(time_q)+"\' WHERE vk_id = "+str(chat_id)+";")
            self.connection.commit()

    def get_question_result(self,chat_id,question_number):
        """ Получить данные пользователя """
        with self.connection:
            self.cursor.execute("SELECT z"+question_number+"_0,z"+question_number+"_1 FROM question WHERE vk_id="+str(chat_id)+";")
            return self.cursor.fetchone()

    def insert_user(self, chat_id,first_name='',last_name='',username=''):
        """добавление пользователя в базу данных"""
        with self.connection:
            sql="""INSERT INTO question(vk_id,first_name,last_name,username)
    ...
    def update_code(self, chat_id,code):
        """записываем результат в базу данных"""
        with self.connection:
            self.cursor.execute("UPDATE question SET code="+str(code)+" WHERE vk_id = "+str(chat_id)+";")
            self.connection.commit()
